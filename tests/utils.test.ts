...
describe('getClassNames', () => {
  test('className', () => {
    const classNames = getClassNames(`
      <div class="flex items-center p-10" transform=""></div>
      <div className="flex items-center p-10"></div>
    `)
  ...
  test('clsx', () => {
    const classNames = getClassNames(`
      const btn = clsx('flex', 'items-center', 'p-10', x === 100 ? 'bg-blue-500' : 'bg-red-500')
      <div className={clsx("flex items-center p-10")}></div>
      <div className="flex items-center p-10"></div>
      <div className={x === 100 ? "flex" : clsx("flex items-center p-10")} style={{ transform: "" }}></div>
    `)
    expect(classNames).toEqual([
      {
        start: 24,
  ...
  test('classnames', () => {
    const classNames = getClassNames(`
      const btn = classnames('flex', 'items-center', 'p-10', true ? 'bg-blue-500' : 'bg-red-500')
      <div className={classnames("flex items-center p-10")}></div>
    `)
    expect(classNames).toEqual([
      {
        start: 30,
